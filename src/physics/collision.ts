...
const collisionResponse = (
  ...
  const distanceNormalised = new Point(
    dPoint.x / distanceRaw,
    dPoint.y / distanceRaw,
  );

  const relativeVelocity = new Point(
  ...
  const speed =
    relativeVelocity.x * distanceNormalised.x +
    relativeVelocity.y * distanceNormalised.y;
  ...
  return new Point(
    impulse * distanceNormalised.x,
    impulse * distanceNormalised.y,
  );
};
