...
(async () => {
  ...
  const spriteSize = isMobile.phone
    ? SPRITE_WIDTH_MOBILE
    : SPRITE_WIDTH_DESKTOP;
  const spriteRadius = isMobile.phone
    ? SPRITE_RADIUS_MOBILE
    : SPRITE_RADIUS_DESKTOP;
  const impulsePower = isMobile.phone
    ? IMPULSE_POWER_MOBILE
    : IMPULSE_POWER_DESKTOP;

  // Add game objects (character sprites, mouse point etc.)
  const { hero, enemy, mousePoint } = await loadGameObjects(
    app,
    spriteSize,
    spriteRadius,
  );
